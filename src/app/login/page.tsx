...
import { getProfiles, getLastActiveProfile, login, type Profile } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const preferred = getLastActiveProfile() ?? (all.length === 1 ? all[0] : null);
    if (preferred) {
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!selected) return;
    if (login(selected.id, name, password)) {
      refresh();
      router.replace("/");
    } else {
      setError("That name/password doesn't match this profile.");
    }
  }
          ...
          {profiles.map((p) => (
            <li key={p.id} className="border-b border-border last:border-b-0">
              <button
                ...
                className="flex min-h-12 w-full items-center justify-between gap-3 px-5 py-3 text-left transition-colors hover:bg-accent-soft"
              >
